...
> **開發進度／暫緩項目請見 [TASKS.md](TASKS.md)，本文件只定規格，不追蹤開發狀態。** 2026-07-06 決定暫緩三個月（預計 2026-10 後再評估）的項目：§3/§8.5 AI 額度後端執行、§5.9 Google 聯絡人 API 匯入、§5.8 照片搜尋子功能、§8.4 Stripe 訂閱整合——這四項規格本身不變，只是本輪不實作。
...
- 計入 AI 使用次數配額（比照 §5.5 其他生成式功能），不論語音或文字輸入皆計 1 次，無額外費用歸類問題
- **語音輸入技術路徑（v1 定案）**：前端錄音後將音訊檔直接傳給 Cloud Function，由支援多模態音訊輸入的 Gemini 模型一次完成「轉錄＋推理萃取」，不另外整合裝置端語音轉文字服務（如 Web Speech API）。理由：避免 Safari/iOS 對 Web Speech API 支援不穩定的相容性問題，且省去獨立語音轉文字服務的整合與成本歸類複雜度，架構更單純
...
- **首頁「今日摘要」統計卡（v1 定案，新增）**：首頁另外呈現待處理提醒、最近互動、本月生日、最近新增聯絡人四項統計數字（UI 呈現見 §11.3）。其中「最近新增聯絡人」定義為建立時間（`Contact.createdAt`）落在**過去 72 小時內**的聯絡人，用時間窗口而非「取最新固定 N 筆」計算——超過 72 小時即不再計入，即使該聯絡人仍是資料庫裡最新建立的幾筆之一，避免久未新增聯絡人時畫面仍顯示著早已不算「最近」的舊資料

### 5.5 AI 功能（付費差異化功能）
1. **名片 OCR**：拍照/上傳名片圖片 → Gemini 解析出姓名、職稱、公司、電話、Email，帶入「新增聯絡人」完整表單供使用者確認/修改後才儲存（v1 定案：辨識完不直接寫入資料庫，需經過完整表單確認，原則與 5.7 批次匯入一致）。**儲存成功後（v1 定案，新增）**：系統自動將這張名片照片（已裁切去除背景，僅保留名片本體）存入該聯絡人的照片集；若 AI 於同一次辨識中判斷名片上印有本人照片，另外裁出該張照片並設為第一張（作為大頭照），名片全圖則作為第二張留存參考；裁切失敗或未偵測到合理邊界時，直接退回存入未裁切的原圖，不中斷新增流程
...
- 每位聯絡人詳情頁提供一個按鍵（例如「搜尋網路資料」），觸發後：
  1. Cloud Function 使用 Gemini + Google Search 工具，以聯絡人的姓名、公司、職稱等已知資訊為關鍵字搜尋；**搜尋範圍明確涵蓋社群媒體平台（LinkedIn、Facebook），不僅是一般網頁搜尋**（v1 定案，新增）
...
- **欄位回填（v1 定案，新增）**：除摘要文章外，AI 同時嘗試從搜尋結果中抽取可回填聯絡人欄位的結構化資訊（職稱、公司、生日、電話、Email、LinkedIn/Facebook/Twitter 連結），隨該筆研究紀錄一併呈現。使用者於確認畫面**逐欄位勾選**是否套用，**畫面預設不會有任何欄位被預先勾選**——即使聯絡人該欄位目前是空白的也不例外，避免使用者因「反正是空的就套用」心態而略過核實。電話、Email 因誤植/誤認風險較高，Prompt 需採用更高的信心門檻，只有來源明確標示為本人所有時才抽取，避免張冠李戴
- **持續累積**：每次觸發都會新增一筆帶時間戳記的研究紀錄，使用者可看到該聯絡人隨時間累積的所有網路研究摘要（例如換工作、發表新聞等變化都保留歷史軌跡），而不是只保留最新一筆；使用者可針對單筆研究紀錄執行**刪除**（v1 定案，新增），避免清單隨時間無限增長佔用畫面空間——刪除為使用者手動操作，不影響上述「預設持續累積、不自動覆蓋」的行為
- 資料結構：`Contact.researchLog: ResearchEntry[]`，見第 7 節資料模型
- 計入 AI 使用次數配額（每次觸發計 1 次，文字摘要、欄位抽取與照片搜尋算同一次觸發，不重複扣配額）
...
interface ResearchEntry {
  ...
  extractedFields?: Partial<Pick<Contact, 'role' | 'company' | 'phone' | 'email' | 'linkedin' | 'facebook' | 'twitter' | 'birthday'>>;
  // 本次搜尋額外抽取、可能可以回填聯絡人欄位的結構化資訊（v1 定案，新增，見 §5.8「欄位回填」）。
  // 純記錄用途，是否真的寫回 Contact 對應欄位需使用者在 UI 逐項確認，不會自動覆蓋。
  createdAt: Timestamp;             // 用於呈現時間軸，累加不覆蓋；使用者可個別刪除單筆紀錄（見 §5.8）
}
...
- **狀態管理定案採用 Zustand**（v1 定案，取代原先「Zustand 或 Context+useReducer 均可」的模糊選項，避免舊版 prop drilling）：Zustand 在 React Native 上為同一套 API，未來 RN 版可直接複用同一套 store 邏輯；並可搭配內建 `persist` middleware，供 §5.11 離線優先所需的本地狀態快取使用
...
4. 網路身分研究摘要功能，是否需要限制單一聯絡人的研究紀錄筆數上限（避免無限累積佔用資料庫空間），待設計時確認（v1 已新增使用者可手動刪除單筆紀錄，見 §5.8，可部分緩解此問題，但是否仍需自動上限尚待確認）。
...
9. 5.3a 的「重要性星級建議」呈現方式（例如快速記錄確認畫面內的一個核取選項，還是額外產生一筆待處理建議）待設計 UI 時確認，v1 原則上不自動覆寫 `Contact.importance`，僅呈現建議供使用者主動採納。
10. Interaction 改為獨立集合並支援 `contactIds` 多對多綁定後，Firestore Security Rules 與查詢方式（例如「取得某聯絡人的所有互動」需改為 `where('contactIds', 'array-contains', contactId)`）需在技術設計階段確認索引與效能影響。
...
- 首頁另外呈現「今日摘要」2×2 統計卡格線（待處理提醒、最近互動、本月生日、最近新增聯絡人，定義見 §5.4）。點擊任一統計卡會**捲動至畫面下方對應的清單區塊**，不彈出新畫面；若對應清單目前沒有資料而未顯示於畫面上，點擊該卡不動作（v1 定案，新增）
