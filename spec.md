...
1. 提醒功能的觸發規則（例如「超過 90 天沒互動」自動建議 vs 使用者手動設定日期）細節待設計 UI 時確認。
...
5. 照片搜尋屬肖像權敏感功能，是否需要額外的法律免責聲明文案、或限制此子功能僅付費版可用（而非全部使用者），待確認。
...
8. AI 主動提醒的觸發門檻（生日前幾天開始提醒、「多久沒互動」預設幾天）具體數字待定，暫定生日前 3 天、90 天無互動。
9. 主動提醒的執行頻率與 Cloud Scheduler 成本（使用者數量增加後每日全量掃描的執行時間與費用）待上線前試算，若成本過高可能需改為分批/分時掃描。
10. 星級（1～5 星）與 AI 主動提醒門檻的對應規則（例如幾星對應多久沒互動就提醒）具體數字待設計時確認，暫定僅先實作「星級可排序/篩選」，門檻連動邏輯可視為 v1.x 增強項目。
11. 5.3a 語音／自然語言快速記錄：語音輸入是走裝置端語音轉文字（如瀏覽器 Web Speech API）再送文字給 Gemini，還是直接送音訊給支援音訊輸入的 Gemini 模型，兩者成本與準確度差異待技術驗證（AI 配額計算方式已定案：不論內部呼叫方式，每次觸發計 1 次，見第 3 節）。
12. 5.3a 的「重要性星級建議」呈現方式（例如快速記錄確認畫面內的一個核取選項，還是額外產生一筆待處理建議）待設計 UI 時確認，v1 原則上不自動覆寫 `Contact.importance`，僅呈現建議供使用者主動採納。
13. Interaction 改為獨立集合並支援 `contactIds` 多對多綁定後，Firestore Security Rules 與查詢方式（例如「取得某聯絡人的所有互動」需改為 `where('contactIds', 'array-contains', contactId)`）需在技術設計階段確認索引與效能影響。
