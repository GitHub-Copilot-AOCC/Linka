...
**§11.5/§11.6 後曾表示開發到此為止**，之後使用者又提出三項新需求（標籤複選＋隨時新增、Excel 匯出、首頁近況摘要），已完成見下方表格。其餘 4 項功能（AI 額度後端執行、Google 聯絡人 API 匯入、照片搜尋、Stripe）已於 2026-07-06 明確決定暫緩三個月，見下方「⏸️ 暫緩三個月」表格。
...
## ⏸️ 暫緩三個月（2026-07-06 決定，預計 2026-10 之後再評估）

使用者明確決定以下 4 項**不再列入本輪開發範圍**，三個月後再重新評估是否要做。這不是「之後隨手做」的待辦，是刻意暫停：

| 章節 | 功能 | 現況 |
|---|---|---|
| §3 / §8.5 | AI 額度後端真正執行（增量/擋額度） | 前端唯讀顯示已完成；`functions/src/index.ts` 完全沒有 `usage`/`quota`/`periodId` 相關程式碼，呼叫 AI 功能目前不會被擋也不會計數，額度形同虛設。**暫緩期間額度不會生效，需注意 Gemini API 用量/費用沒有任何上限機制**，若暫緩期間用量異常需另外用 Google Cloud Console 監看，不能依賴這個功能 |
| §5.9 | Google 聯絡人 API 匯入 | 只做了 vCard(.vcf)，Google People API OAuth scope 沒接 |
| §5.8 | 照片搜尋子功能 | 需要額外圖片搜尋 API/憑證（例如 Google Custom Search JSON API + CSE ID），本專案目前未設定 |
| §8.4 | Stripe 訂閱整合 | 完全沒開始（前端 Checkout 頁面、後端 webhook 都沒有） |

到期後重新評估時，先回頭讀這四行備註跟 spec.md 對應章節，不要假設現況跟三個月前一樣（尤其 Gemini/Firebase 相關套件版本可能已變動）。
